...
void append_value(node **tail, uint val) {
...
void insert_value(node *previous_node, uint val){
...
node *lookup(node *head, uint val) {
...
void shift_right(node **head, node **tail){
    // Shifts the linked list right
    insert_head(head, create_node((*tail)->val));
    remove_tail(tail);
}
